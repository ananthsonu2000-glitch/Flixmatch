...
async function fetchFromProvider(
  ...
  try {
    ...
    if (res.status === 429 && attempt < 2) {
      ...
      await sleep(Number.isFinite(retryAfterSec) && retryAfterSec > 0 ? retryAfterSec * 1000 : 1500 * (attempt + 1));
      return fetchFromProvider(tmdbId, mediaType, attempt + 1);
    }
...
export async function enrichCandidates(
  ...
  for (const c of budgeted) {
    const enriched = await fetchFromProvider(c.tmdb_id, c.media_type);
    if (enriched) {
